...
const Games = () => {
  ...
  useEffect(() => {
    const fetchGamesAndRelatedData = async () => {
      try {
        ...
        // Load both FBS and FCS teams (only once)
        if (teams.length === 0) {
        ...
        // Use the selected filterYear in API calls
        const gamesData = await teamsService.getGames(filterYear, week);
        ...
        const weatherData = await teamsService.getGameWeather(filterYear, week);
        setWeather(weatherData);

        const mediaData = await teamsService.getGameMedia(filterYear, week);
        setMedia(mediaData);

        const linesData = await teamsService.getGameLines(filterYear);
        setLines(linesData);
      } catch (err) {
  ...
  // Filter games based on search term (teams or conferences)
  const filteredGames = games.filter((game) => {
    ...
    // Check team names
    if (
      game.homeTeam.toLowerCase().includes(lowerSearch) ||
      game.awayTeam.toLowerCase().includes(lowerSearch)
    ) {
      return true;
    }

    // Optionally check team conferences (assuming teams have a 'conference' field)
    const homeTeam = teams.find(
      (t) => t.school.toLowerCase() === game.homeTeam.toLowerCase()
    );
    const awayTeam = teams.find(
      (t) => t.school.toLowerCase() === game.awayTeam.toLowerCase()
    );

    if (
      (homeTeam && homeTeam.conference && homeTeam.conference.toLowerCase().includes(lowerSearch)) ||
      (awayTeam && awayTeam.conference && awayTeam.conference.toLowerCase().includes(lowerSearch))
    ) {
      return true;
    }
    return false;
  ...
  // WeatherIcon component (as before)
    ...
    const icons = {
      ...
      cloudy: (
        <svg {...iconStyle} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <path
            d="M19.5 14.5c0 2.2-1.8 4-4 4H6.5c-2.8 0-5-2.2-5-5 0-2.3 1.6-4.3 3.8-4.8.5-2.5 2.7-4.2 5.2-4.2 2.3 0 4.3 1.5 5 3.6.4-.1.9-.1 1.3-.1 2.2 0 4 1.8 4 4 0 .3 0 .5-.1.8.9.5 1.5 1.5 1.5 2.6 0 1.7-1.3 3.1-2.9 3.1h-.8"
      ...
      rain: (
        <svg {...iconStyle} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <path
            d="M19 12.5c0 1.9-1.5 3.5-3.5 3.5H6c-2.5 0-4.5-2-4.5-4.5 0-2.1 1.4-3.8 3.4-4.3.5-2.2 2.4-3.7 4.7-3.7 2.1 0 3.9 1.3 4.5 3.2.4-.1.8-.1 1.2-.1 2 0 3.6 1.5 3.6 3.5 0 .2 0 .5-.1.7.8.5 1.4 1.3 1.4 2.3 0 1.5-1.2 2.7-2.6 2.7h-.8"
      ...
      clear: (
        <svg {...iconStyle} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle
            ...
            filter="drop-shadow(0 0 2px rgba(255, 160, 0, 0.6))"
          />
          <path
            d="M12 4v2M12 18v2M6 12H4M20 12h-2M7.05 7.05l1.4 1.4M15.55 15.55l1.4 1.4M7.05 16.95l1.4-1.4M15.55 8.45l1.4-1.4"
            stroke="#FF9800"
            strokeWidth="2"
            strokeLinecap="round"
          />
      ...
      <header className="header">
        ...
        <div className="filters-container">
          ...
          <div className="year-selector">
            <label>Year:</label>
            <select value={filterYear} onChange={handleYearChange} className="year-dropdown">
