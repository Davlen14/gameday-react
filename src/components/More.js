...
const More = () => {
  ...
  const getStateCentroid = (coordinates) => {
    // For simple polygons
    ...
    return getStateCentroid([coordinates[0]]);
  };

  // Create team territories by assigning states to closest team
  useEffect(() => {
    if (usStatesData && filteredTeams.length > 0) {
      try {
        // Create a deep copy of the states data
        ...
        statesCopy.features.forEach(state => {
          ...
          // Assign team to state
          if (closestTeam) {
            state.properties.teamId = closestTeam.id;
            state.properties.teamName = closestTeam.school;
            state.properties.teamColor = closestTeam.color && closestTeam.color !== "null" ? closestTeam.color : "#333";
            state.properties.teamLogo = closestTeam.logos?.[0];
          }
        });
  ...
  const onStateClick = (e, feature) => {
    const team = filteredTeams.find(t => t.id === feature.properties.teamId);
    if (team && mapRef.current) {
  ...
  const onEachState = (feature, layer) => {
    // Add click event
    layer.on({
      click: (e) => onStateClick(e, feature)
    });
    
    // Add tooltip
    layer.bindTooltip(`${feature.properties.name} - ${feature.properties.teamName || 'No team'}`, {
      sticky: true
    });
  };
  ...
  // Create a team logo icon
  const createTeamLogoIcon = (team) => {
    return L.divIcon({
