...
const TeamPlayerModal = ({
  ...
  useEffect(() => {
    const fetchTeamLogosAndGrades = async () => {
      ...
      // Fix here: Check if player and player.position exist before accessing
      if (player && player.position === "QB" && player.statsData) {
        try {
  ...
  // Fix here: Safe check for playerPosition
  ...
  const generateSnapData = () => {
    ...
    return {
      ...
      positionBreakdown: [
        { position: playerPosition || "Unknown", snaps: 700 },
